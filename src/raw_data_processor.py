...
class RawDataProcessor:
    ...
    def process_raw_data(prob_config: ProblemConfig):
        ...
        #######################find category from raw data + capture data################
        training_data = RawDataProcessor.apply_category_features(training_data,
...
def find_category_index(prob_config: ProblemConfig):
        logging.info("start process find index")
        captured_x = pd.DataFrame()
        for file_path in prob_config.captured_data_dir.glob("*.parquet"):
            print(file_path)
            captured_data = pd.read_parquet(file_path)
            captured_x = pd.concat([captured_x,captured_data])
        training_data = pd.concat([pd.read_parquet(prob_config.raw_data_path), captured_x])
        training_data, category_index = RawDataProcessor.build_category_features(
            training_data, prob_config.categorical_cols
        )
...
if __name__ == "__main__":
    ...
    # find_category_index(prob_config)
